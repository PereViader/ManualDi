...
namespace ManualDi.Sync.Tests
{
    public class TestDiContainerScope
    {
        ...
        public void TestTransient()
        {
            ...
            var container = new DiContainerBindings().Install(b =>
            {
                b.Bind<object>().TransientFromMethod(factoryMethodDelegate).Inject(injectionDelegate);
            }).Build();

            var resolution1 = container.Resolve<object>();
            var resolution2 = container.Resolve<object>();

            // Verify that the injection delegate and factory method were called three times -
            // once for build, then once for each resolution.
            injectionDelegate.Received(3).Invoke(Arg.Any<object>(), Arg.Any<IDiContainer>());
            factoryMethodDelegate.Received(3).Invoke(Arg.Any<IDiContainer>());
        }
    }
}
